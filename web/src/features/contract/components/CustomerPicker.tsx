    ...
    <div ref={combo.boxRef} className={`relative ${className ?? ""}`}>
      ...
      {value && !combo.editing ? (
        <div className="flex items-center gap-1">
          <Input id={combo.inputId} readOnly aria-label={label || "Customer"} className="cursor-pointer"
        ...
        <Input id={combo.inputId} ref={combo.inputRef} aria-label={label || "Customer"} placeholder={value && selected ? `${selected.code} · ${selected.name} — type to replace...` : placeholder}

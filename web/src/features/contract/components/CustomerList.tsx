...
import { customersQuery } from "../hooks/contractQueries";
...
export function CustomerList() {
  ...
  const listQ = useQuery(customersQuery({ q: q || undefined, status: status === "All" ? undefined : status, page, size: 20 }));
    ...
    { accessorKey: "code", header: "CODE" },
    {
      ...
      cell: ({ row }) => (
        <div>
          ...
          <div className="text-xs text-muted-foreground">{row.original.code} · {row.original.taxCode ?? "—"}</div>
        </div>
      ),
    },
    {
      id: "primary", header: "PRIMARY CONTACT",
      cell: ({ row }) => row.original.primaryContact ? <div className="text-sm">{row.original.primaryContact.fullName}<div className="text-xs text-muted-foreground">{row.original.primaryContact.email}</div></div> : <span className="text-xs text-muted-foreground">—</span>,
    },
    { accessorKey: "status", header: "STATUS", cell: ({ row }) => <StatusBadge status={row.original.status} /> },
    {
      ...
      cell: ({ row }) => (
        <div className="space-x-1">
          <Button size="sm" variant="outline" onClick={() => setViewContacts(row.original)}>View</Button>
          {canWrite && <Button size="sm" variant="outline" onClick={() => onEdit(row.original)}>Edit</Button>}
          {canWrite && row.original.status === "ACTIVE" ? <Button size="sm" variant="destructive" onClick={() => suspendMut.mutate(row.original.id)}>Suspend</Button> : canWrite && <Button size="sm" onClick={() => activateMut.mutate(row.original.id)}>Activate</Button>}
        </div>
      ),
    },
  ], [canWrite]);
    ...
    <div className="space-y-4">
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          {listQ.isLoading ? <div className="text-sm text-muted-foreground">Loading...</div> : listQ.isError ? <div className="text-sm text-destructive">{getApiErrorMessage(listQ.error, "Failed")}</div> : <DataTable columns={columns} data={customers} emptyMessage="No customers" />}

...
7 business services + gateway + 1 external mock + 1 frontend. Each maps to a requirement section; none is speculative.
...
**Audit (4.10) — deliberate non-service:** each service writes its own `audit_log` table **in the same transaction** as the change (same column convention everywhere, defined in registry). Rationale: in-transaction write = audit can never be lost or diverge (4.10 "không phụ thuộc dữ liệu hiển thị hiện tại"); per-entity lookup (per contract / price list / statement / signing session) is served by the owning service. A central audit service is explicitly rejected as over-engineering.

## 4. How services connect (locked decisions)

Decisions are numbered `D#` so artifacts and critics can reference them.

- **D1 — Sync vs async split.** Synchronous REST (JSON, OpenAPI) for queries and commands that need an immediate answer; async events over the broker for anything that is a *consequence* (notifications, status reactions, audit-relevant milestones). At least the workflow→notification and esign flows are async (satisfies req §6).
...
- **D4 — Workflow start is idempotent.** Owner services call `POST /workflow-instances` with `(document_type, document_id)`; a partial unique index (one *active* instance per document) makes double-submit harmless (5.5 double-submit row).
...
- **D11 — Two-layer authorization.** Gateway: JWT validity + coarse role check. Services: contextual checks — especially workflow-service verifying *the caller is the assignee of the current step* (APR-01: "kiểm tra role là chưa đủ").
...
- **D13 — Tech stack for design artifacts.** Backend assumed FastAPI (Python) for concreteness in sequence diagrams; nothing in the diagrams may depend on it beyond naming.
...
**Event catalog (seed — full schema in registry):** `workflow.instance_started`, `workflow.step_assigned`, `workflow.step_actioned` (approve/reject/revision + comment), `workflow.completed {outcome}`, `esign.session_completed {result}`, `document.expiring`, all with envelope `{event_id, event_type, occurred_at, actor, document_type, document_id, payload}`.
...
5. Sync API dependency matrix: endpoint-level (method, path, purpose).
6. Common column conventions (id/created/updated/version), audit_log table shape, outbox table shape.
...
| 1.2 | `design/01-architecture.md` + `diagrams/architecture.drawio` | Container diagram: gateway, 7 services + their DBs/schemas, Redis, RabbitMQ, esign-mock, frontend; sync arrows (labeled with purpose) vs async arrows (labeled with event types). Written justification for: DB-per-schema (D12), customer-contract merge, audit-as-table, broker choice (D2). **Also decide attachment/file storage here** (4.2 tệp đính kèm, CTR-02, e-sign payload rendering) — default recommendation: attachment metadata table in contract-service + files on a mounted volume; no object-store service at student scope. Done when every D# decision is visible or cited. |
...
| 2.2 | contract | Contract, addendum (own workflow + effective_date, CTR-07), attachment, customer + status field per state machine; edit-allowed states (CTR-01) as a check constraint or app rule — decide & note. Customer suspension (4.1). Audit_log. |
...
| Step | File | Screens |
|---|---|---|
| 4.1 | `ui-shared` | Login; app shell (role menu, notification bell); **task inbox** ("hồ sơ cần xử lý" — the central approver screen); notification list; document history tab (audit per entity, 4.10); workflow progress stepper (done/current/remaining steps + current holder, 4.7). |
...
- `audit_log` per service, one shape (registry §5.6): `entity_type, entity_id, action, actor_id, actor_name, before_status, after_status, changes JSONB, note, created_at` — written in-transaction.
- Outbox table (registry shape) only in event-emitting services.
- Each `db-*.md` ends with a rule-mapping table ("PRC-03 → exclusion constraint on price_list_version…").
- ER drawio: tables as entity boxes grouped per service; crow's-foot within service; dashed annotation arrows for cross-service UUID references (labeled "ref: contract-service.contract.id").

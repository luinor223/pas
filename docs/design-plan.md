...
| Step | Diagram | Covers |
|---|---|---|
| 3.1 | `seq-02-outbox-audit-notification` | The mechanics drawn ONCE: business tx (status + status_history + outbox rows) → relay poll/claim → RabbitMQ → audit-service PK dedup + notification-service `processed_event` dedup → user reads/marks read; plus the two-source History tab (local `status_history` vs remote `AuditInternal.ListRecords`). Later diagrams `ref` this instead of redrawing. |
| 3.2 | `seq-01-auth` | Login → JWT issue; authenticated request through the gateway (blacklist, D11 two layers); the §M1 permission-cache sweep and fail-closed behaviour; logout blacklists the jti. |

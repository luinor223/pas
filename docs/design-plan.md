...
- Shapes: `shape=umlLifeline` (+ `participant=umlActor|umlBoundary|umlControl|umlEntity`), activation bars, `shape=umlFrame` for `alt`/`opt`/`loop`/`ref`, `shape=note` in a right-hand gutter for invariants.

...
- **`outbox` scope**: `document.expiring` is still a **direct publish** (D9 — a lost warning re-fires next run, so outbox there is overhead). The outbox exists for `audit.recorded` (D15): every customer/contract/addendum action, including the system-applied addendum effects above, is written to `outbox` in the same transaction as the change.

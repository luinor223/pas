...
- **Status flips**: consumes `workflow.completed` + `esign.session_completed` (processed_event); notifications about statements ride workflow/esign events (§4).
...
- **`outbox`, two uses**: (1) audit (D15) — calculate, controlled edit, reconcile, submit, publish, cancel and adjustment creation are written as `audit.recorded` in the same transaction; PAY-05 (no direct edit after Approved/Signed) is enforced by status checks, but *defended* by this trace. (2) submit wiring (registry §9 footnote ⁵, D4) — submit sets `status = SUBMITTED` **and** writes an `event_type = 'workflow.start_requested'` row to the same `outbox`, in the same local transaction; a background dispatcher retries `WorkflowInternal.StartInstance` until it succeeds. Calling workflow-service before the local commit was considered and rejected system-wide (design-plan D4) — a successful remote call followed by a failed local write would orphan a live, activated workflow instance on a statement still `RECONCILED`. While dispatch is pending the statement is genuinely `SUBMITTED` with no instance yet (UI: "Submitted — workflow initialization pending"). Cancelling from this state runs §6's retry-until-resolved algorithm: atomic outbox cancel only if never claimed (a stale claim is still treated as potentially in flight, never cancelled directly — its owner may be paused, not dead), else `WorkflowInternal.CancelInstance` retried on `NOT_FOUND`, forcing a stale claim's dispatch to resolution itself before retrying — the statement stays SUBMITTED until one branch definitively resolves.

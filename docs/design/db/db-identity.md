...
- **Permissions are resolved from a cached map, not carried in the JWT** (registry §6; supersedes the earlier `permissions[]`-claim decision): the token carries `roles[]`, and each service resolves role → permissions from a Redis-cached map refreshed from identity. There is still no `/permissions` lookup endpoint on the hot path — contract, operations, billing and workflow check their permission (`contract:cancel_active`, `volume:edit_locked`, `statement:cancel_approved`, `workflow:configure`) against the resolved set, and identity checks its own `user:manage` the same way rather than special-casing itself. Difference from the claim approach: a grant/revoke via "Phân quyền" takes effect on the next cache refresh rather than at the user's next login.

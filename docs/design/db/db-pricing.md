...
- **Status flips**: consumes `workflow.completed` (processed_event); scheduler flips APPROVED→EFFECTIVE→EXPIRED + emits `document.expiring` (D9 direct publish, not via outbox — a lost warning re-fires next run).
- **`status_history` (D17)**: append-only, per `price_list_version`, written in the same transaction as each status flip. Note the two *system* transitions it captures that have no user behind them — the in-transaction predecessor truncation (§9³) and the scheduler's EFFECTIVE/SUPERSEDED/EXPIRED flips (`trigger_kind = S`). Those are exactly the ones that look inexplicable later without a log, since PRC-05 means a version's prices can never be edited to explain them.
- **`outbox`, two uses**: (1) audit (D15) — version lifecycle actions (submit, revise, approve, the in-transaction predecessor truncation, the scheduler's EFFECTIVE/SUPERSEDED/EXPIRED flips) are written as `audit.recorded` in the same transaction; this is what makes PRC-04/05 defensible after the fact, the price history a statement was built against is reconstructible. (2) submit wiring (registry §9 footnote ⁵, D4) — submit sets `status = SUBMITTED` **and** writes an `event_type = 'workflow.start_requested'` row to the same `outbox`, in the same local transaction; a background dispatcher retries `WorkflowInternal.StartInstance` until it succeeds. Calling workflow-service before the local commit was considered and rejected system-wide (design-plan D4) — a successful remote call followed by a failed local write would orphan a live, activated workflow instance on a version still `DRAFT`. While dispatch is pending the version is genuinely `SUBMITTED` with no instance yet (UI: "Submitted — workflow initialization pending"). Cancelling from this state runs §6's retry-until-resolved algorithm: atomic outbox cancel only if never claimed (a stale claim is still treated as potentially in flight, never cancelled directly — its owner may be paused, not dead), else `WorkflowInternal.CancelInstance` retried on `NOT_FOUND`, forcing a stale claim's dispatch to resolution itself before retrying — the version stays SUBMITTED until one branch definitively resolves.
...
| 4.4 versions & history | version rows + `audit.recorded` → audit-service |

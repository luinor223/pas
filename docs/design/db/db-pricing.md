...
- **`outbox` for audit (D15)**: version lifecycle actions — submit, revise, approve, the in-transaction predecessor truncation and the scheduler's EFFECTIVE/SUPERSEDED/EXPIRED flips — are written as `audit.recorded` to `outbox` in the same transaction. This is what makes PRC-04/05 defensible after the fact: the price history a statement was built against is reconstructible.
